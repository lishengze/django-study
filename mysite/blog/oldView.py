...
def update_srv(request):
	...
	if request.method == "POST":
		...
		###发布时间
		exec_time = start_time + 1200
		###1:返回脚本内容，其他则返回脚本的简单介绍
		args = 1
		if not myFile:
		...
		if rsp.startswith(cfg.TIP_BODY_UPDATE):
			...
			if args == 1:
				return HttpResponse(updateinfo.fileinfo.replace("\n", "<br/>"))
			else:
				return HttpResponse(updateinfo.cmdinfo.replace("\n", "<br/>"))
		else:
			return HttpResponse(rsp.replace("\n", "<br/>"))
	else:
		return index(request)
...
def exec_update(request):
	...
	if request.method != '':
		...
		daemaon_ip, daemon_port = getKey(ENV_KEY)
		sock = sock_conn(daemaon_ip, daemon_port)
		print(genExecUpdateHead(filename))
...
##查询所有未发布更新包
@csrf_protect
def query_all_unupdate(request):
	#if request.method == 'POST':
	if request.method != '':
		#从POST请求中获取查询关键字
		###
