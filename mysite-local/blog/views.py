...
def getKey(key):
	daemaon_ip = ENV_DICT[key][1]
	daemon_port = ENV_DICT[key][2]
...
class ViewMain(object):
	...
	def get_ajax_func(self, path):
		...
		ajax_func_dict = {
			...
			'Get_All_UnUpdate_VersionInfo': self._ajax_func.get_all_unupdate_verinfo,
			'Delete_Update_File': self._ajax_func.delete_update_file,
			'Release_Update': self._ajax_func.release_update,
			'Release_Update_Complete': self._ajax_func.release_update_complete,
	...
	def get_upload_file_env_key(self, filePath):
		...
		elif filePath.find('Release_Update_Complete') != -1:
			exec_time = pathArray[len(pathArray)-1]
			ENV_KEY = pathArray[len(pathArray)-2]
			filePath = filePath[0:filePath.find('Release_Update_Complete') + len('Release_Update_Complete') + 1]
...
class HandleFileAjaxFunc(object):
	...
	def downloadFile(self, sock, file_src, file_dst):
		...
		try:
			...
			# file_dst_array = file_dst.split('/')
			# if len(file_dst_array) > 1:
			# 	file_dst_dir = '/'.join(file_dst_array[:len(file_dst_array)-1])
			# 	output_msg('file_dst_dir: ', file_dst_dir)
	...
	def download_file(self, request):
		...
		try:
			...
			sock = sock_conn(daemaon_ip, daemon_port)
			file_src = file_name
			file_dst = os.path.join(cfg.WORK_PATH_TEMP, file_name)
			print ('file_src:  ' + file_src, 'file_dst:  ' + file_dst)
			downloadData = self.downloadFile(sock, file_src, file_dst)
			sock.close()

			if downloadData['status'] == cfg.CMD_SUCC:
				rsp = downloadData['data'].decode('gb2312')
			# try:
			# 	f = open(file_dst, 'r')
			# 	if f:
			...
			output_msg('Exception: ', error)
			output_msg('Django notifyDaemon failed!', traceback.format_exc())
	...
	# 一键发布
	def release_update_complete(self, request, ENV_KEY, delay_time):
		rsp = ""
		status = SUCCESS_FLAG
		error = ""
		upload_time = -1
	...
	def update_srv(self, request, ENV_KEY, delay_time):
		...
		try:
			...
			daemaon_ip, daemon_port = getKey(ENV_KEY)
			sock = sock_conn(daemaon_ip, daemon_port)
			sock.send(genUpdateHead(filepath, exec_time) + cfg.TIP_INFO_EOF)
			rsp = recv_end(sock)
			sock.close()
			upload_time = int(time.time()) - start_time

			output_msg('Update_Srv Info:　', rsp)

			if rsp.startswith(cfg.TIP_BODY_UPDATE):
				info = rsp[len(cfg.TIP_BODY_UPDATE):]
				updateinfo = UpdateInfo()
				updateinfo.decode(info)
				if args == 1:
					rsp = updateinfo.fileinfo
				else:
					rsp = updateinfo.cmdinfo
    ...
    # 发布更新文件到远端
	def release_update(self, request):
		rsp = ""
		status = SUCCESS_FLAG
		error = ""
		try:
	...
	# 查询所有未发布更新包
	def get_all_unupdate_verinfo(self, request):
		rsp = ""
		status = SUCCESS_FLAG
		error = ""
		try:
			...
			if "" in rsp:
				rsp.remove("")
			rsp = rsp[1:]
			output_msg('All UnUpdate Version Info: ', rsp)
	...
	# 删除某个未发布的更新包
	def delete_update_file(self, request):
		rsp = ""
		status = SUCCESS_FLAG
		error = ""
		try:
			...
			daemaon_ip, daemon_port = getKey(ENV_KEY)
			sock = sock_conn(daemaon_ip, daemon_port)
			sock.send(genDelUpdateHead(filename) + cfg.TIP_INFO_EOF)
			rsp = recv_end(sock)
			sock.close()
			output_msg('Delete Update: ', rsp)
...
class ProcessAjaxRspData(object):
	def process_rsp_result(self, origin_data, data_type):
		...
		if failed_data == '':
			...
			return {'data': dict_data, 'type': data_type, 'status': status}
		else:
			status = FAILED_FLAG
			return {'data': failed_data, 'type': 'Failed', 'status': status}
			...
			if tmpdata[1].find('verControl Usage:') != -1:
				...
				for value in tmpdata:
					if value.find(verControl_SUCC_flag) != -1:
						failed_data = ''
						break
		elif len(tmpdata) > 1 and tmpdata[1].find('exec doExit!') != -1:
			failed_data = 'exec doExit!'
		else:
			failed_data = ''
		return failed_data
...
class SimulationData(object):
	 ...
	 def get_simu_nft_task_state_data(self):
		 data = []
		 return data

	 def get_simu_nft_task_result_data(self):
		 data = []
		 return data

	 def get_simu_all_ntf_task_result_data(self):
		 data = []
		 return data

	 def get_simu_all_ntf_task_state_data(self):
		 data = []
		 return data
...
class TaskAjaxFunc(ProcessAjaxRspData, SimulationData):
	...
	def request_real_time_info_task(self, request):
		...
		try:
			...
			original_rsp_data = self.get_simu_real_time_info_task_data()
			...
			rsp = trans_rsp_data['data']
			data_type = trans_rsp_data['type']
			if trans_rsp_data['status'] == FAILED_FLAG:
				error = trans_rsp_data['data']
				status = FAILED_FLAG
	...
	def request_real_time_versionCtr_task(self, request):
		...
		try:
			...
			data_type = 'Failed'
			error = exception.args
			output_msg('Django notifyDaemon failed!', error)
			output_msg('Django notifyDaemon failed!', traceback.format_exc())
	...
	def request_ntf_task(self, request):
		...
		try:
			...
			status = FAILED_FLAG
			error = exception.args
			output_msg('Exception: ', exception.args)
			output_msg('Django notifyDaemon failed : ', traceback.format_exc())
	...
	def get_nft_task_state(self, ENV_KEY, task_id = 0):
		...
		try:
			...
			rsp = self.get_simu_nft_task_state_data()
			...
			if task_id == 0:
				...
				output_msg('Task List: ', task_list)
			rsp = task_list
			if task_list == []:
				status = FAILED_FLAG
				error = '查询不到任务信息'
	...
	def get_nft_task_result(self, ENV_KEY, task_id = 0):
		...
		try:
			...
			task_result = self.get_simu_nft_task_result_data()
	...
	def get_all_ntf_task_result(self, request):
		...
		try:
			...
			while index < len(task_data):
				...
				if task_info['status'] == FAILED_FLAG:
					error = task_info['error']
					rsp = task_info['rsp_data']
					status = task_info['status']
					break

				if task_info['rsp_data']['state'] == 3:
					task_result[task_id] = self.get_nft_task_result(ENV_KEY, task_id)
					if task_result[task_id]['status'] == SUCCESS_FLAG :
						cur_result = self.process_rsp_result(task_result[task_id]['rsp_data'], task_type[task_id])
						task_result[task_id] = {
						'rsp_data': cur_result['data'],
						'status': cur_result['status'],
						'type': cur_result['type'],
						'error': ''}
				elif task_info['rsp_data']['state'] == 1:
					task_result[task_id] = {
					'rsp_data': '',
					'status': FAILED_FLAG,
					'type': cur_result['type'],
			...
			if status == SUCCESS_FLAG:
				output_msg('Task Result', task_result)
				rsp = task_result
	...
	def get_all_version(self, request):
		...
		try:
			...
			if info in version_list:
				print info
			rsp = version_list
			output_msg('Get_All_Version: ', rsp)
	...
	def get_version_difference(self, request):
		...
		try:
			...
			daemaon_ip, daemon_port = getKey(ENV_KEY)
			rsp = self.get_simu_version_difference_data()
...
class GetHtmlObject(object):
	...
	def test_database_data(self):
		...
		output_msg('group_array', group_array)
		output_msg('user_array', user_array)
